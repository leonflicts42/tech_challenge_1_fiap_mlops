...
class TestSemanticNormalizerFit:
    def test_fit_retorna_self(
        self, normalizer: SemanticNormalizer, base_df: pd.DataFrame
    ) -> None:
        result = normalizer.fit(base_df)
        assert result is normalizer

    def test_fit_com_y_retorna_self(
        self, normalizer: SemanticNormalizer, base_df: pd.DataFrame
    ) -> None:
...
class TestSemanticNormalizerTransform:
    def test_no_internet_service_vira_no(
        self, normalizer: SemanticNormalizer, base_df: pd.DataFrame
    ) -> None:
    ...
    def test_nao_modifica_valores_ja_corretos(
        self, normalizer: SemanticNormalizer, base_df: pd.DataFrame
    ) -> None:
        out = normalizer.transform(base_df)
    ...
    def test_nao_modifica_dataframe_original(
        self, normalizer: SemanticNormalizer, base_df: pd.DataFrame
    ) -> None:
    ...
    def test_inconsistencia_sem_internet_corrige_servicos(
    ...
    def test_sem_inconsistencia_nao_altera_nada(
        self, normalizer: SemanticNormalizer
    ) -> None:
        df = pd.DataFrame(
            [
    ...
    def test_sem_internet_service_col_ignora_validacao(
        self, normalizer: SemanticNormalizer
    ) -> None:
        df = pd.DataFrame(
            [{"online_security": "No internet service", "tech_support": "Yes"}]
        )
        out = normalizer.transform(df)
        assert out["online_security"].iloc[0] == "No"
