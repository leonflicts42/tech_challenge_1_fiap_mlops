"""Testes para ChurnMLPv2 e ChurnMLPInference — models/mlp2.py."""
...
from models.mlp2 import ChurnMLPInference, ChurnMLPv2
...
class TestChurnMLPv2:
    ...
    def test_forward_shape_correto(self, input_dim: int, x: torch.Tensor, batch_size: int) -> None:
        model = ChurnMLPv2(input_dim=input_dim, hidden_dims=[64, 32])
        model.eval()
        with torch.no_grad():
            out = model(x)
